...
const TablaHome = (props) => {
    ...
    const lastday = function (year = 2023, month) {
        ...
        <div className='table__global__container'>
            {(props.date) ?
                ((props.date !== -1) & (props.date !== "{}") ?
                    <Table headers={["TIPO DE REPORTE:", "FACTURACIÓN"]} content={["CAMPAÑA:", client + " INBOUND", "DESDE", "01/" + props.date + "/2023", "HASTA", lastday("2023", props.date) + "/" + props.date + "/2023"]} />
